...
#include<time.h>
using namespace std;
// structure for GSS bucket with 12 bit fingerprint. 
...
{
public:
	unsigned char src_fp[room_num]; // the higher 8 bit of each fingerprint of src node is stored here  
	unsigned char dst_fp[room_num]; // the higher 8 bit of each fingerprint of dst node is stored here  
	...
	void get_room_src(int src_rfp[room_num]) // get the src fingerprint of each room 
	{
		for (int i = 0; i < room_num; i++)
		{
			src_rfp[i] = (((unsigned int)src_fp[i]) << 4 | (remain[i] >> 4)); // concatenate the higher 8 bit stored in src_fp and the lower 4 bit stored in remain. 
		}
	...
	void get_room_dst(int dst_rfp[room_num]) // get the dst fingerprint of each room 
	{
		for (int i = 0; i < room_num; i++)
		{
			dst_rfp[i] = (((unsigned int)dst_fp[i]) << 4 | (remain[i] &0xF)); // concatenate the higher 8 bit in dst_fp and the lower 4 bit in reamin
		}
	}
	...
	{
		src_fp[r_num] = (fp>>4);	// the higher 8 bit
		remain[r_num] = (remain[r_num] | ((fp & 0xF) << 4)); // the lower 4 bit
	}
...
{
public:
	unsigned char src_fp[room_num];
	unsigned char dst_fp[room_num];
