...
#include "setting.h"
#include"../common/hashtable_with_bitmap.h"
...
public:
	...
	GSS(unsigned long long matrix_size_, int range_, int candidate_, bool use_hashtable, int fp_size = fp_len, int table_size = initial_size)
	{
		r = range_;;
		candidate = candidate_;
		m = sqrt(matrix_size_ / room_num / r / r);
		range = pow(2, fp_size)*m;
		use_ht = use_hashtable;
		if (use_hashtable)
			ht = new hashtable_bs(table_size);
		else
			ht = new hashtable_bs(initial_size);
	...
	void insert(string s, string d, int w)
	{
		...
		for (int k = 0; k < candidate; k++)
		{
			...
			for (int j = 0; j < room_num; j++){
				...
				else if (src == 0 && dst == 0)
				{
					...
					matrix[index][pos].weight[j] = w;
					if (use_ht){
						ht->increase(code_s, index, r, Src_Direct);
						ht->increase(code_d, index, r, Dst_Direct);
					}
					return;
				}
			}
		...
		if (use_ht){
			ht->increase(code_s, -1, r, Src_Direct);
			ht->increase(code_d, -1, r, Dst_Direct);
		}
	}
	...
	void insert_with_delete(string s, string d, int w)
	{
		...
		if (find_empty)// if there is any empty pos in the matrix, try to find the edge in the buffer, and if find, move it from the buffer to the matrix
		{
			...
			if (new_w == 0 && use_ht)
			{
				...
				if (use_ht && old_w == 0){
					ht->increase(code_s, empty_index, r, Src_Direct);
					ht->increase(code_d, empty_index, r, Dst_Direct);
				}
	...
	void successor(string v, vector<string> &vec)
	{
		...
		bs_room* v_room = ht->get_room(code_v);
		...
		if (v_room) {
			...
			for (int i = 0; i < r*r; i++)
			{
				if (v_room->vb.get_bit(i, r, Src_Direct))
				{
					//int count = 0;
					int index1 = i / r;
					int index2 = i%r;
	...
	void precursor(string v, vector<string> &vec)
	{
		...
		if (v_room) {
			...
			for (int i = 0; i < r*r; i++)
			{
				if (v_room->vb.get_bit(i,r, Dst_Direct)){
					int index1 = i / r;
					int index2 = i%r;
					int p2 = (h2 + tmp2[index2]) % m;
					for (int j = 0; j < m; j++)
